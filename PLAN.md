...
**Суммарная оценка:** ~62–79 ч (этапы 0–11, включая 10.5).
...
**Итог:** список блокеров → исправления → «ОК» → **этап 10.5 (доработки)** → **этап 11 (Tauri)**.

---

## Этап 10.5. Доработки web перед Tauri

> **Промежуточный этап** после ревью этапа 10 и **до** обёртки в Tauri.
> Цель: закрыть технический долг и усилить web-версию, чтобы в Tauri не тащить сырой UX и дыры в тестах.

**Оценка:** 6–10 ч

**Предусловие:** ревью перед Tauri принято автором («ОК», 06.07.2026).

**Блокер для этапа 11:** этап 10.5 закрыт по чеклисту ниже + «ОК» автора.

### Quick wins (1–2 ч каждый)

- [ ] **Ошибка сохранения vault в UI** — `app-store.error` пишется в `vault-persistence.ts`, но нигде не показывается; toast/banner в `app-root.tsx`
- [ ] **«Сессия до закрытия» на мобильном** — сейчас `hidden sm:flex` в `session-controls.tsx`; перенести в настройки или показать на 375px
- [ ] **Подтверждение закрытия модалки** — несохранённые данные теряются при клике на backdrop (`modal.tsx`)
- [ ] **E2E: переключение темы** — светлая/тёмная, класс `dark` на `html`
- [ ] **E2E: импорт replace + неверный пароль `.vault`**
- [ ] **E2E: smoke CRUD лицензии** — создать, отредактировать, архив (хелпер `addRealLicense` уже есть)
- [ ] **Синхронизировать `APP_RELEASED_AT`** — `src/constants/app-info.ts` vs даты этапов

### Средний приоритет (3–5 ч)

- [ ] **Vitest: crypto roundtrip** — `vault-crypto.ts`, serializer с фото (`Uint8Array`)
- [ ] **E2E: категории CRUD**, фото (upload/галерея), поиск RU↔EN
- [ ] **E2E: авто-блокировка по таймеру** (порог 1 мин в настройках)
- [ ] **Замена `window.confirm`** на модалки в стиле приложения (демо, настройки, форма, категории, галерея)
- [ ] **Переименовать `DemoPanel` → `LicensesPanel`** — фактически страница лицензий
- [ ] **Минимальный CI** — GitHub Actions: `npm run test:all` + `npm run build` на push

### Из Roadmap v2 — подтянуть до Tauri

- [ ] **Теги-автодополнение** в форме лицензии (данные уже в store)
- [ ] **Категории в верхней навигации** (сейчас только «Управление…» в сайдбаре)

### Оставить на Tauri / v2

- Авто-бэкап в файл (логичнее с нативным path picker)
- Автозапуск Windows (часть этапа 11)
- Audit log, календарь, CSV-импорт, несколько vault — Roadmap v2

### Итог

Чеклист закрыт → «ОК» → **этап 11 (Tauri)**.
...
**Предусловие:** этап 10.5 (доработки web) закрыт автором.
...
5. **Ревью UI и регрессия** — обязательные вехи (не пропускать):
   ...
   - после **этапа 10** → перед **этапом 10.5** и **Tauri** (этап 11).
...
flowchart LR
    ...
    E6 --> E10
    E10 --> E105[Этап 10.5 Доработки]
    E105 --> E11[Этап 11 Tauri]
```
